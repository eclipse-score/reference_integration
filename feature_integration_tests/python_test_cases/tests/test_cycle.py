...
class FitTest:
    ...
    def container(self, docker: Container):
        logging.debug("Setup container")
        yield docker
        logging.debug("Teardown container")
    ...
    def exec_binary(
        ...
        logging.debug(f"Executing binary in container: {command}")
        container.exec_run(command)

    def read_json_file(self, container, file_path) -> Dict[str, Any]:
...
class TestCycle(FitTest):
    ...
    def test_container_is_clean(self, container: Container):
        result = container.exec_run("ls /app/test_data")
        output = result.output.decode()
    ...
    def test_first_cycle(
        self, container: Container, scenario_name: str, test_input: str
    ):
        self.exec_binary(
            container,
        ...
        data = self.read_json_file(container, self.kvs_current_file)
        assert {"cycle_counter": 1} == data

    def test_second_cycle(
        self, container: Container, scenario_name: str, test_input: str
    ):
        self.exec_binary(
            container, scenario_name=scenario_name, scenario_input=test_input
        )
        data_current = self.read_json_file(container, self.kvs_current_file)
        assert {"cycle_counter": 2} == data_current
        data_previous = self.read_json_file(container, self.kvs_previous_file)
        assert {"cycle_counter": 1} == data_previous

    def test_cycling(self, container: Container, scenario_name: str, test_input: str):
        for _ in range(10):
            data_before_exec = self.read_json_file(container, self.kvs_current_file)
            cycle_before_exec = data_before_exec["cycle_counter"]
            self.exec_binary(
                container, scenario_name=scenario_name, scenario_input=test_input
            )
            data_after_exec = self.read_json_file(container, self.kvs_current_file)
