...
import pytest
from daemon_helpers import launch_manager_daemon, signal_process
...
def _resolve_scenario_binary(build_tools: BuildTools, request: pytest.FixtureRequest) -> Path:
    """
    Resolve the scenario binary path via `build_tools`.

    Uses `select_target_path`, which reads the `--rust-target-path`/
    `--cpp-target-path` option already injected by the BUILD target's
    `$(rootpath ...)` args. Falling back to `find_target_path` (which shells
    out to `bazel build`/`bazel cquery`) does not work here: under `bazel
    test`, nested bazel invocations from inside the test sandbox fail.
    """
...
def _run_persistency_probe(
    ...
    request : pytest.FixtureRequest
        Current test's fixture request, for resolving the `--rust-target-path`/
        `--cpp-target-path` cmdline option via `build_tools.select_target_path`.
    kvs_dir : Path
        Directory for KVS storage.
    timeout_s : float
        Execution timeout in seconds.
    """
    scenario_binary = _resolve_scenario_binary(build_tools, request)
...
def _force_kill_supervised_process(pid: int, *, sandbox_privileged: bool) -> tuple[bool, str]:
    """
    Force-kill a supervised process using SIGKILL to trigger recovery.

    Supervised apps run under the daemon's configured sandbox uid/gid when
    sandbox capabilities are granted, so a plain `kill` from the test runner's
    own uid can fail; delegate to `signal_process`, which escalates via
    `sudo -n kill` in that case (see `daemon_helpers.signal_process`).
    """
    return signal_process(str(pid), "-9", sandbox_privileged=sandbox_privileged)
...
class TestLifecyclePersistencyRecoveryContinuity:
    ...
    def test_persistency_continuity_across_sequential_writes(
        ...
        _run_persistency_probe(build_tools, request, kvs_dir, timeout_s=30.0)
    ...
    def test_persistency_recovery_with_daemon_supervision(
        ...
        kill_success, kill_reason = _force_kill_supervised_process(
            supervised_pid, sandbox_privileged=launch_manager_daemon["sandbox_privileged"]
        )
        assert kill_success, f"Failed to kill supervised process {supervised_pid}: {kill_reason}"
    ...
    def test_supervised_app_crash_persistency_recovery(
        ...
        _simulate_probe_crash(build_tools, request, kvs_dir, crash_delay_s=0.5)
