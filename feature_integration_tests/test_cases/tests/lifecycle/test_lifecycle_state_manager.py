...
class TestLifecycleStateManagerIf:
    ...
    def test_status_query_returns_current_run_target(
        ...
        Not tagged with the control-interface requirements above: this test
        checks startup log evidence only, not a response to an activate_target
        request, so it does not prove the control-interface contract itself
        (see module docstring — no query/response API exists to check that).
